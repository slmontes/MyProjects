...
public:
	void TestEpithelialLayerUndergoingFission() throw(Exception)
	{
		...
		double end_time = 10; //250.0; //Set end time
		...
		std::cout << "#" << argc-1 << " arguments supplied.\n" << std::flush;
			   ...
			   if (argc != 4)
			   {
				   std::cerr << "TestGenerateCryptSteadyState::Please input two arguments\n"
								"* Cell-type ratio & Comparative Paneth Cell Stiffness\n" << std::flush;
				   return;
			   }
		...
		for(unsigned index=start_sim; index < start_sim + num_sims; index++){
			...
			for (unsigned i = 0; i<real_indices.size(); i++)
			{
				...
				p_cycle_model->SetTargetProportion(target_proportion+0.1); //Set the division parameter
